# Mapa da documentação

Este documento é um catálogo. O ponto de entrada e roteiro de leitura é [docs/INDICE_DOCUMENTACAO.md](../../INDICE_DOCUMENTACAO.md).

```text
Visão ─┬─ Glossário
       ├─ Regras ──────────┐
       └─ Arquitetura/ADRs ┼─> Feature ─> Plano ─> Código/Testes
Padrões ───────────────────┘                 └─> Documentação
Skills/Prompts ─> modo de execução
```

## Relações

Use links relativos. Evite duplicação: cada fato tem uma origem autoritativa e os demais documentos apontam para ela.

Consulte também o [guia de organização](GUIA_ORGANIZACAO_DOCUMENTACAO.md).

## Catálogo

### Core e operação

- [Contrato](ADF_FRAMEWORK.md)
- [Fluxo](FLUXO_DESENVOLVIMENTO.md)
- [Papéis](PAPEIS_DAS_IAS.md)
- [Checklist](CHECKLIST_USO_ADF.md)
- [Mapa de skills](MAPA_SKILLS.md)
- [Índice legado](INDICE_DOCUMENTACAO.md)

### Prompts

- [Iniciar feature](../Prompts/PROMPT_INICIAR_FEATURE.md)
- [Revisar entrega](../Prompts/PROMPT_REVISAR_ENTREGA.md)

### Templates

- [Feature](../Templates/TEMPLATE_FEATURE.md)
- [Plano de implementação](../Templates/TEMPLATE_PLANO_IMPLEMENTACAO.md)
- [Checklist de implementação](../Templates/TEMPLATE_CHECKLIST_IMPLEMENTACAO.md)
- [Revisão](../Templates/TEMPLATE_REVISAO.md)
- [Prompt de execução](../Templates/TEMPLATE_PROMPT_EXECUCAO.md)
- [Prompt de revisão](../Templates/TEMPLATE_PROMPT_REVISAO.md)
- [ADR](../Templates/TEMPLATE_ADR.md)
- [Padrão](../Templates/TEMPLATE_PADRAO.md)
- [Regra de negócio](../Templates/TEMPLATE_REGRA_NEGOCIO.md)
- [Visão de projeto](../Templates/TEMPLATE_VISAO_PROJETO.md)

### Áreas do projeto consumidor

- [Projeto](../../Projeto/README.md)
- [Arquitetura](../../Arquitetura/README.md)
- [Padrões](../../Padroes/README.md)
- [Regras de negócio](../../RegrasNegocio/README.md)
- [Features](../../Features/README.md)
- [Revisões](../../Revisoes/README.md)
- [Exemplo de feature](../../Samples/FEATURE_EXEMPLO.md)
